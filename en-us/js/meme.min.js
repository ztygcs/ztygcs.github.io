...
changeMode();storePrefers();});}},{once:true});window.addEventListener('storage',function(event){if(event.newValue==='dark'){changeModeMeta('dark');}else{changeModeMeta('light');}
...
function initIndex(){let request=new XMLHttpRequest();request.open("GET","../../en-us/search.json");request.responseType="json";request.addEventListener("load",function(event){let documents=request.response;if(!documents)
{console.error("Search index could not be downloaded, was it generated?");searchDone();return;}
lookup={};index=lunr(function(){const language="en-us";if(language!="en"&&lunr.hasOwnProperty(language)){this.use(lunr[language]);}
...
let title=document.createElement("h1");title.id="search-results";title.className="list-title";if(results.length==0){title.textContent="没有找到关于「{}」的结果".replace("{}",term);}
else if(results.length==1){title.textContent="找到 1 条关于「{}」的结果".replace("{}",term);}
else{title.textContent="找到 13579 条关于「{}」的结果".replace("{}",term).replace("13579",results.length);}
...
return result;}},{once:true});;function siteTime(){window.setTimeout("siteTime()",1000);var seconds=1000
...
if(diffYears==0){document.getElementById("sitetime").innerHTML=" "+diffDays+" 天 "+diffHours+" 小时 "+diffMinutes+" 分钟 "+diffSeconds+" 秒"}else{document.getElementById("sitetime").innerHTML=" "+diffYears+" 年 "+diffDays+" 天 "+diffHours+" 小时 "+diffMinutes+" 分钟 "+diffSeconds+" 秒"}}
siteTime()